...
\begin{frame}{Strain rate $\dot\varepsilon$}
  \begin{figure}
  \includegraphics[width=7cm]{figures/fig_4_04}
  \end{figure}
  shear strain $\varepsilon = \frac{1}{2}\left( \frac{\Delta x}{\Delta z}\right)$
  \begin{displaymath}
    \dot \varepsilon= 1/2 \gamma
  \end{displaymath}
  shear strain 
\begin{displaymath}
\dot \varepsilon = 1/2\,\left( \frac{\Delta x / \Delta z}{\Delta t}\right) = 1/2\,\left( \frac{\Delta u}{\Delta z}\right)
\end{displaymath}
\end{frame}
...
\begin{frame}
  ...
  \begin{block}{We need to define}
    \begin{itemize}
   \item a relationship between stress and strain, $\tau = f(\dot \varepsilon)$
   \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}
  ...
  \begin{displaymath}
    \dot \varepsilon = A(T,p)\,\tau^{n-1}\tau
  \end{displaymath}
  \\
  \begin{equation*}
    \begin{array}{rcll}
      A(T,p) & = & A_{0} e^{-(Q+pV)/T} &\qquad \text{Arrhenius law}
    \end{array}
  \end{equation*}
  \begin{itemize}
