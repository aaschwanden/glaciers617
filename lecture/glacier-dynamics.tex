...
\begin{frame}
 ...
 \begin{block}{Solution: Continuums Mechanics \& Rheology}
  ...
  \end{itemize}
  \note{usefull stuff, not only for ice dynamics. recently applied by Truffer to solve a surface mass balance problem}
\end{block}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item identify $\mathbf{t}_{\mathbf{n}} = \mathbf{T}\mathbf{n}$
      \item $\mathbf{T}$ is the \alert{Chauchy stress tensor}
    \end{itemize}    
\end{frame}
...
\begin{frame}
  ...
  \begin{block}{Divergence Theorem}
    ...
    \end{equation}
    \note{The sum of all sources minus the sum of all sinks gives the net flow out of a region}
  \end{block}
  \begin{block}{Reynolds Tranport Theorem}
    ...
    \end{equation}
    \note{What was already there plus what goes in minus what goes out is what is there}
  \end{block}
\end{frame}
\end{document}
